  ...
  return (
    <main className="about-manga-container">
      ...
      <div className="manga-page">
        ...
        {/* --- 1. IDENTITY：熱意と感謝へ繋ぐ --- */}
        <Link href="/research" className="area-identity" style={{ textDecoration: 'none', color: 'inherit' }}>
          <section className="manga-panel link-panel">
            ...
            <div className="video-wrapper">
              <iframe 
                ...
                style={{ pointerEvents: 'none' }} // パネル全体をクリックさせるため
              ></iframe>
            </div>
          ...
          <div className="mission-content">
            ...
            <div className="mission-quote">
              <p>
                ...
                開発・テスト・CS、どのような役割であっても、それが創作を支えることに繋がるなら、私にとってこれ以上の喜びはありません。
              </p>
            </div>
...
const aboutStyles = `
  .about-manga-container {
    background-color: #f0f0f0; min-height: 100vh; padding: 80px 20px;
    background-image: radial-gradient(#ccc 1.5px, transparent 1.5px);
    background-size: 25px 25px; transition: background-color 1s ease;
  }

  .fixed-nav { position: fixed; top: 20px; left: 20px; z-index: 1000; }
  .back-top-btn { 
    background: white; border: 4px solid black; padding: 10px 20px; 
    font-weight: 900; box-shadow: 5px 5px 0px black; cursor: pointer;
  }

  .manga-page { max-width: 1100px; margin: 0 auto; display: grid; gap: 25px; grid-template-columns: repeat(12, 1fr); }

  .manga-panel {
    background: white; border: 8px solid black; box-shadow: 15px 15px 0px black;
  ...
  .panel-header-badge {
    ...
    padding: 5px 15px; font-weight: 900; z-index: 10; font-size: 0.8rem;
  }
  ...
  /* カラフルワールド発動時の変化 */
  .colorful-world-active .manga-panel {
    box-shadow: 15px 15px 0px rgba(135, 206, 235, 0.8) !important;
    border-color: #ff69b4 !important;
  }

  .main-passion-title { 
    font-size: clamp(2.5rem, 8vw, 5.5rem); font-weight: 1000; text-align: center; 
    margin: 50px 0 30px; font-style: italic; letter-spacing: -3px; line-height: 1;
  }
  .video-wrapper { width: 100%; aspect-ratio: 16/9; border-top: 6px solid black; border-bottom: 6px solid black; background: #000; }
  .panel-caption { padding: 15px; font-weight: 900; text-align: right; font-style: italic; color: #e63946; }
  ...
  .guts-inner-text { 
    ...
    background: rgba(0,0,0,0.7); padding: 30px; border: 4px solid white; margin: 20px;
  }
  ...
  .baan-img { width: 50vw; max-width: 700px; animation: baan-pop 0.6s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; }
  @keyframes baan-pop {
